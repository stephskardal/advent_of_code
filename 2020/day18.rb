...
# Part 2
def solve_new_math(str)
  if(str.match(/(.*)(\([0-9+*]*\))(.*)/))
    solve_new_math("#{$1}#{solve_new_math($2[1..-2])}#{$3}")
  else
    eval_new_math(str)
  end
end

def eval_new_math(str)
  while(str.match(/((.*)([*+]))?([0-9]+\+[0-9]+)(.*)/)) do
    replacement = eval($4)
    str = "#{$1}#{replacement}#{$5}"
  end
...
(8 * 2 + 9) * (9 * 2 + 5) + 7 * (2 * 2 * 9 + 9 + 7) * 2".split("\n").map{ |d| d.gsub(/ /, '') }
end

puts "part 1: #{inputs.map{ |input| solve(input) }.sum}"
puts "part 2: #{inputs.map{ |input| solve_new_math(input) }.sum}"
