def part_1(data)
  ...
  data.select { |input| ((required & input.split('-').map{ |b| b.split(':')[0] }).size == required_size) }.size
end

def part_1(data)
  required_fields = %w(byr iyr eyr hgt hcl ecl pid)
  required = {
    byr: Proc.new { |b| b.to_i >= 1920 && b.to_i <= 2002 },
    iyr: Proc.new { |b| b.to_i >= 2010 && b.to_i <= 2020 },
    eyr: Proc.new { |b| b.to_i >= 2020 && b.to_i <= 2030 },
    hgt: Proc.new { |b| (b.match(/cm$/) && b.to_i >= 150 && b.to_i <= 193) || (b.match(/in$/) && b.to_i >= 59 && b.to_i <= 76) },
    hcl: Proc.new { |b| !b.match(/^#[a-f0-9]{6}$/).nil? },
    ecl: Proc.new { |b| %w(amb blu brn gry grn hzl oth).include?(b) },
    pid: Proc.new { |b| !b.match(/^[0-9]{9}$/).nil? },
    cid: Proc.new { |b| true }
  }
  required_size = required_fields.size
  data.select do |input|
    ((required_fields & input.split('-').map{ |b| b.split(':')[0] }).size == required_size) &&
      input.split('-').map { |b| required[b.split(':')[0].to_sym].call(b.split(':')[1]) }.all? { |obj| obj }
  end.size
end

# inputs pre-processed in vim to sub extra lines and format for ruby parsing
def inputs
%w(
